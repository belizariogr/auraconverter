...
- Server file logs live under `AURA_DATA_DIR/logs/` (written by `server.ts` / `fileLog.ts`, not by Electron). Failed narration blocks are also saved under `logs/failed-chunks/`.
